...
echo "     Prediction: 0.5s horizon (25 steps @ 0.02s)"
...
if ! ros2 pkg executables px4_offboard | grep -q mpc; then
    ...
    if ! grep -q "mpc.*mpc:main" src/ROS2_PX4_Offboard_Example/px4_offboard/setup.py; then
        echo "  MPC entry point missing from setup.py!"
...
echo ""
echo "  WARNING: Phase 5 bypasses PX4 attitude controller!"
...
if ros2 pkg executables px4_offboard | grep -q mpc; then
    ...
    echo ""
    echo "  Manual run attempt (if entry point exists):"
