...
#         # In the MPCCascadeDemo.__init__() method, around line 600-650, replace the MPC initialization section:

#         # Replace the MPC initialization section (around lines 480-550) with this:

#         # Initialize MPC controller with GP enhancement
#         if CASADI_AVAILABLE:
#             # Check environment variables for GP mode
#             collect_data = os.getenv('GP_DATA_COLLECTION', 'false').lower() == 'true'
#             use_gp_model = os.getenv('GP_USE_MODEL', 'false').lower() == 'true'
#             model_path = os.getenv('GP_MODEL_PATH', '/home/grandediw/ros2_px4_offboard_example_ws/gp_models/gp_model_20251119_030043.pkl')
            
#             if collect_data:
#                 # Data collection mode - save to CSV, don't use GP
#                 self.get_logger().info("📊 Data collection mode enabled")
#                 self.mpc = QuadrotorMPC(dt=0.02, N=25, use_gp=False)
#                 if GP_AVAILABLE:
#                     self.mpc.gp_dynamics = SimpleQuadrotorGP(max_data_points=2000)
#                     self.mpc.gp_enhanced = None
#                 self.get_logger().info("💾 Flight data will be saved to ~/gp_datasets/")
                
#             elif use_gp_model and os.path.exists(model_path):
#                 # GP-enhanced mode - load and use trained model
#                 self.get_logger().info("🧠 GP-enhanced mode enabled")
#                 self.mpc = QuadrotorMPC(dt=0.02, N=25, use_gp=True)
                
#                 if GP_AVAILABLE:
#                     # Initialize GP for model loading
#                     self.mpc.gp_dynamics = SimpleQuadrotorGP(max_data_points=1000)
                    
#                     # Try to load trained model using different methods
#                     model_loaded = False
                    
#                     # Method 1: Try the load_model method if it exists
#                     if hasattr(self.mpc.gp_dynamics, 'load_model'):
#                         try:
#                             if self.mpc.gp_dynamics.load_model(model_path):
#                                 model_loaded = True
#                                 self.get_logger().info(f"✅ GP model loaded via load_model(): {model_path}")
#                         except Exception as e:
#                             self.get_logger().warn(f"⚠️ load_model() failed: {e}")
                    
#                     # Method 2: Manual loading with pickle (fallback)
#                     if not model_loaded:
#                         try:
#                             import pickle
...
#                             # Load model data manually
#                             self.mpc.gp_dynamics.gp_model = model_data['gp_model']
#                             self.mpc.gp_dynamics.is_trained = True
#                             self.mpc.gp_dynamics.training_count = model_data.get('training_count', 1)
                            
#                             model_loaded = True
#                             self.get_logger().info(f"✅ GP model loaded manually: {model_path}")
                            
#                         except Exception as e:
#                             self.get_logger().error(f"❌ Failed to load GP model manually: {e}")
                    
#                     # Initialize GP-enhanced wrapper if model loaded successfully
#                     if model_loaded:
#                         try:
#                             # Check if your SimpleGPEnhancedMPC exists
#                             if 'SimpleGPEnhancedMPC' in globals():
#                                 self.mpc.gp_enhanced = SimpleGPEnhancedMPC(
#                                     self.mpc.gp_dynamics, 
#                                     confidence_threshold=0.15  # Conservative threshold
#                                 )
#                             else:
#                                 # Create a simple wrapper if SimpleGPEnhancedMPC doesn't exist
#                                 self.mpc.gp_enhanced = self.mpc.gp_dynamics
                            
#                             # Log model info
#                             stats = self.mpc.gp_dynamics.get_stats()
#                             self.get_logger().info(f"📊 GP Model Info:")
#                             self.get_logger().info(f"   - Training samples: {stats.get('data_points', 'unknown')}")
#                             self.get_logger().info(f"   - Training iterations: {stats.get('training_iterations', 'unknown')}")
#                             self.get_logger().info(f"   - Model ready for enhanced dynamics")
                            
#                         except Exception as e:
#                             self.get_logger().error(f"❌ Failed to initialize GP-enhanced wrapper: {e}")
#                             model_loaded = False
                    
#                     # Fallback if loading failed
#                     if not model_loaded:
#                         self.get_logger().warn(f"⚠️ Failed to load GP model: {model_path}")
#                         self.get_logger().warn("⚠️ Using nominal dynamics only")
#                         self.mpc.use_gp = False
#                         self.mpc.gp_enhanced = None
#                 else:
#                     self.get_logger().warn("⚠️ GP not available, using nominal dynamics")
#                     self.mpc.use_gp = False
                    
#             else:
#                 # Standard mode - no GP, no data collection
#                 self.get_logger().info("📊 Standard MPC mode (no GP)")
#                 self.mpc = QuadrotorMPC(dt=0.02, N=25, use_gp=False)
#                 if GP_AVAILABLE:
#                     self.mpc.gp_dynamics = SimpleQuadrotorGP(max_data_points=800)
#                     self.mpc.gp_enhanced = None
#         else:
#             self.get_logger().error("❌ CasADi not available - MPC disabled")
#             self.mpc = None
#         # === GP DATASET LOGGING SETUP ===
#         # We'll save the GP training data from self.mpc.gp_dynamics to a CSV
...
#         self.gp_csv_path = os.path.join(base_dir, f"gp_mpc_data_{timestamp}.csv")
#         self.get_logger().info(f"📁 GP dataset will be saved to: {self.gp_csv_path}")
...
#     def save_gp_dataset(self):
#         """Save GP training data (if available) to CSV for offline training."""
#         if (
#             self.mpc is not None and
#             self.mpc.use_gp and
#             self.mpc.gp_dynamics is not None
#         ):
#             try:
#                 self.mpc.gp_dynamics.save_dataset(self.gp_csv_path)
#                 self.get_logger().info(f"📁 Saved GP dataset to {self.gp_csv_path}")
#             except Exception as e:
#                 self.get_logger().error(f"❌ Failed to save GP dataset: {e}")
#         else:
#             self.get_logger().info("ℹ️ No GP dataset to save (GP not used or no data).")
...
#             # # Train the GP occasionally once you have enough data
#             # if (self.mpc.gp_dynamics is not None and
#             #     len(self.mpc.gp_dynamics.X_train) >= 500):
                
#             #     # Example: train every 200 new samples
#             #     if len(self.mpc.gp_dynamics.X_train) % 200 == 0:
#             #         self.get_logger().info(
#             #             f"🧠 Training GP offline: "
#             #             f"{len(self.mpc.gp_dynamics.X_train)} samples collected"
#             #         )
#             #         self.mpc.gp_dynamics.train_gp()
...
#         Kp_att = 3.2  # Proportional gain (rad/s per rad)
#         Ki_att = 0.6  # Integral gain (rad/s per rad*s)
#         Kd_att = 0.6  # Derivative gain (rad/s per rad/s)
...
#             # Calculate reference trajectory (this returns z=0 for your working case)
#             trajectory_offset, target_yaw = self.calculate_reference_trajectory(flight_time)
            
#             # The MPC trajectory is relative to hover position
#             target_position = np.array([
#                 trajectory_offset[0],  # x offset
#                 trajectory_offset[1],  # y offset  
#                 trajectory_offset[2] + self.takeoff_height  # z offset + hover altitude
#             ])
            
#             # FIX: Update position_setpoint with ABSOLUTE position for logging
...
#             # FIX: Pass the ORIGINAL trajectory to MPC (with z=0)
#             # MPC expects relative coordinates
#             control_outputs = self.update_mpc_control(trajectory_offset, target_yaw)
...
#                 # In the control_loop method, in the Phase 5 logging section, add:
#                 # Enhanced GP learning statistics
#                 if self.mpc.use_gp and self.mpc.gp_dynamics is not None:
#                     gp_stats = self.mpc.gp_dynamics.get_stats()
#                     if gp_stats['is_trained']:
#                         usage_stats = self.mpc.gp_enhanced.get_usage_ratio()
#                         self.get_logger().info(f"   🧠 GP: {gp_stats['data_points']} samples, trained ({gp_stats['training_iterations']} iters), using GP {usage_stats:.0f}% of time")
#                         # Show current prediction uncertainty
#                         if len(self.get_current_state_vector()) >= 6:
#                             current_state_6d = self.get_current_state_vector()[:6]
#                             test_control = np.array([0, 0, 0, 0])
#                             uncertainty = self.mpc.gp_dynamics.get_uncertainty(current_state_6d, test_control)
#                             self.get_logger().info(f"      Current uncertainty: {uncertainty:.4f} (threshold: {self.mpc.gp_enhanced.confidence_threshold})")
#                     else:
#                         self.get_logger().info(f"   📊 GP: {gp_stats['data_points']} samples (model loaded but not active)")
...
#     finally:
#         # 💾 Save GP dataset before shutting down
#         try:
#             mpc_cascade_demo.save_gp_dataset()
#         except Exception as e:
#             print(f"⚠️ Error while saving GP dataset on shutdown: {e}")
...
Integration Flow:
================
1. At each control step, build a nominal / warm-start trajectory.
2. GP predicts dynamics residuals d_k ≈ f_GP(x_k, u_k) along that trajectory.
3. These residuals enter the MPC dynamics:
      x_{k+1} = x_k + dt * (f_nom(x_k,u_k) + d_k)
4. Geometric control allocation converts accelerations to body rates.
5. PX4 executes low-level attitude/rate control.
"""
...
class QuadrotorMPC:
    ...
    def __init__(self, dt=0.02, N=25, use_gp=True):
        ...
        # Scale factor for GP residuals in dynamics (can be tuned)
        self.gp_residual_gain = 0.1 # before 0.1
    ...
    def setup_mpc_problem(self):
        ...
        # CasADi function for nominal dynamics
        self.f = ca.Function("f", [states, controls], [rhs])

        # === PARAMETERS ===
        # P = [x0 (6), target_pos (3), gp_residuals (6*N)]
        self.n_params = self.n_states + 3 + self.n_states * self.N
        P = ca.SX.sym("P", self.n_params)
        self.idx_residual_start = self.n_states + 3
        ...
        # Cost weights
        Q_pos = ca.diag([50.0, 50.0, 80.0])  # Position tracking
        Q_vel = ca.diag([12.0, 12.0, 18.0])  # Velocity damping
        R = ca.diag([2.0, 2.0, 1.0, 8.0])    # Control effort
        ...
        for k in range(self.N):
            # --- Cost ---
            pos_error = X[:3, k] - P[self.n_states : self.n_states + 3]
            vel_error = X[3:6, k]
            ...
            # --- GP residual for this stage (as parameter) ---
            start = self.idx_residual_start + k * self.n_states
            stop = start + self.n_states
            d_k = P[start:stop]  # shape (6,)

            # Dynamics with residual:
            # x_{k+1} = x_k + dt * (f_nom(x_k,u_k) + d_k)
            x_next = X[:, k] + self.dt * (self.f(X[:, k], U[:, k]) + d_k)

            g.append(X[:, k + 1] - x_next)

        # Terminal cost
        pos_error = X[:3, -1] - P[self.n_states : self.n_states + 3]
        vel_error = X[3:6, -1]
        obj += 3 * ca.mtimes([pos_error.T, Q_pos, pos_error])
        obj += 2 * ca.mtimes([vel_error.T, Q_vel, vel_error])
        ...
        for _ in range(self.N):
            # ax, ay, az, yaw_rate
            self.lbx += [-4.0, -4.0, -5.0, -1.0]
            self.ubx += [4.0, 4.0, 8.0, 1.0]

        # Constraint bounds (initial condition + dynamics)
        self.lbg = [0] * (self.n_states * (self.N + 1))
        self.ubg = [0] * (self.n_states * (self.N + 1))

    def add_training_data(self, state_prev, control_prev, state_current):
        """Add training data for GP learning from state transitions."""
        if self.gp_dynamics is None:
            return

        # Use first 6 states (x,y,z,vx,vy,vz)
        state_6d_prev = state_prev[:6] if len(state_prev) >= 6 else state_prev
        state_6d_current = state_current[:6] if len(state_current) >= 6 else state_current
        control_4d = control_prev[:4] if len(control_prev) >= 4 else control_prev

        self.gp_dynamics.add_training_data(state_6d_prev, control_4d, state_6d_current, self.dt)
    
    def _build_gp_residuals(self, mpc_state):
        """Build GP residuals correctly - GP predicts state residuals, not dynamics residuals"""
        D = np.zeros((self.n_states, self.N), dtype=float)

        if not (self.use_gp and self.gp_dynamics is not None and self.gp_dynamics.is_trained):
            return D

        # Use previous solution as trajectory guess if available
        if self.previous_solution is not None:
            X_guess = self.previous_solution["X"]  # shape (6, N+1)
            U_guess = self.previous_solution["U"]  # shape (4, N)
        else:
            X_guess = np.tile(mpc_state.reshape(-1, 1), (1, self.N + 1))
        ...
        for k in range(self.N):
            ...
            try:
                # GP predicts STATE residual for next step
                state_residual_mean, _ = self.gp_dynamics.predict_residual(x_k, u_k)
                state_residual_mean = np.asarray(state_residual_mean).reshape(-1)

                if state_residual_mean.shape[0] >= self.n_states:
                    # ✅ CORRECT: Convert state residual to dynamics residual
                    dynamics_residual = state_residual_mean / self.dt
                    
                    # Apply only to acceleration components
                    d_acc = dynamics_residual[3:6]
                    d_acc = self.gp_residual_gain * d_acc
                    D[3:6, k] = d_acc
    ...
    # def _build_gp_residuals(self, mpc_state):
    #     """
    #     Build a (6, N) array of GP residuals d_k.

    #     By default, we put residual only on the acceleration components (indices 3:6),
    #     i.e. d_k[0:3] = 0, d_k[3:6] ≈ residual on [vx_dot, vy_dot, vz_dot].
    #     """
    #     D = np.zeros((self.n_states, self.N), dtype=float)
    ...
    #         U_guess = self.previous_solution["U"]  # shape (4, N)
    #     else:
    #         # Fallback: constant state and zero controls
    #         X_guess = np.tile(mpc_state.reshape(-1, 1), (1, self.N + 1))
    #         U_guess = np.zeros((self.n_controls, self.N))
    ...
    #         try:
    #             residual_mean, _ = self.gp_dynamics.predict_residual(x_k, u_k)
    #             residual_mean = np.asarray(residual_mean).reshape(-1)

    #             # If GP outputs at least 6D residual, use last 3 as accel residual
    #             if residual_mean.shape[0] >= self.n_states:
    #                 d_acc = residual_mean[3:6]
    #             else:
    #                 # Otherwise take whatever is available (best-effort)
    #                 if residual_mean.shape[0] >= 3:
    #                     d_acc = residual_mean[:3]
    #                 else:
    #                     d_acc = np.zeros(3)
    ...
    #             # Optionally, update GP usage stats if wrapper is present
    #             if self.gp_enhanced is not None and hasattr(
    #                 self.gp_enhanced, "enhanced_dynamics_function"
    #             ):
    #                 _ = self.gp_enhanced.enhanced_dynamics_function(x_k, u_k, self.dt)
    #         except Exception:
    #             # On any GP failure, keep residual zero for this step
    #             pass

    #     return D

    def solve(self, current_state, target_pos, target_yaw=0.0):
        """Solve MPC optimization problem with optional GP-enhanced dynamics."""

        # Extract 6D state [x,y,z,vx,vy,vz]
        mpc_state = np.zeros(6, dtype=float)
        mpc_state[:3] = current_state[:3]
        mpc_state[3:6] = current_state[3:6]

        # --- Build parameter vector P ---
        # First: current state and target position
        p_list = [mpc_state, target_pos]

        # Second: GP residuals for each stage (flattened)
        D = self._build_gp_residuals(mpc_state)  # shape (6, N)
        D_flat = D.flatten()
        p_list.append(D_flat)

        p = np.concatenate(p_list)

        # Initial guess for decision variables
        if self.previous_solution is None:
            # Cold start: all states = current, all controls = 0
            x0 = np.zeros(self.n_states * (self.N + 1) + self.n_controls * self.N, dtype=float)
            for k in range(self.N + 1):
                x0[k * self.n_states : (k + 1) * self.n_states] = mpc_state
            # Controls already zero
        else:
            x0 = self.shift_solution(self.previous_solution, mpc_state)
        ...
        try:
            ...
            X_opt = x_opt[: (self.n_states * (self.N + 1))].reshape(self.n_states, self.N + 1)
            U_opt = x_opt[self.n_states * (self.N + 1) :].reshape(self.n_controls, self.N)
            ...
            # For statistics only: call enhanced_dynamics_function if available
            if (
                self.gp_enhanced is not None
                and self.gp_dynamics is not None
                and self.gp_dynamics.is_trained
                and hasattr(self.gp_enhanced, "enhanced_dynamics_function")
            ):
                _ = self.gp_enhanced.enhanced_dynamics_function(mpc_state, control_output, self.dt)
    ...
    def shift_solution(self, prev_sol, current_state):
        ...
        # Shift states: [x1,...,xN,xN]
        X_shifted = np.hstack([X_prev[:, 1:], X_prev[:, -1:]])
        X_shifted[:, 0] = current_state

        # Shift controls: [u1,...,uN-1,uN-1]
        U_shifted = np.hstack([U_prev[:, 1:], U_prev[:, -1:]])
...
class MPCCascadeDemo(Node):
    def __init__(self):
        ...
        self.position_setpoint_pub = self.create_publisher(
            PointStamped, "/cascade_pid/position_setpoint", 10
        )
        self.position_current_pub = self.create_publisher(
            PointStamped, "/cascade_pid/position_current", 10
        )
        self.velocity_setpoint_pub = self.create_publisher(
            Vector3Stamped, "/cascade_pid/velocity_setpoint", 10
        )
        self.velocity_current_pub = self.create_publisher(
            Vector3Stamped, "/cascade_pid/velocity_current", 10
        )
        self.attitude_setpoint_pub = self.create_publisher(
            Vector3Stamped, "/cascade_pid/attitude_setpoint", 10
        )
        self.attitude_current_pub = self.create_publisher(
            Vector3Stamped, "/cascade_pid/attitude_current", 10
        )
        self.position_error_pub = self.create_publisher(
            Vector3Stamped, "/cascade_pid/position_error", 10
        )
        self.velocity_error_pub = self.create_publisher(
            Vector3Stamped, "/cascade_pid/velocity_error", 10
        )
        self.attitude_error_pub = self.create_publisher(
            Vector3Stamped, "/cascade_pid/attitude_error", 10
        )
        self.control_outputs_pub = self.create_publisher(
            Float64MultiArray, "/cascade_pid/control_outputs", 10
        )
        ...
        # PID integral states
        self.attitude_integral = np.array([0.0, 0.0, 0.0])
        self.integral_max = 0.3
        self.dt_attitude = 0.02

        # Setpoints (for monitoring)
        self.position_setpoint = np.array([0.0, 0.0, self.takeoff_height])
        self.velocity_setpoint = np.array([0.0, 0.0, 0.0])
        self.attitude_setpoint = np.array([0.0, 0.0, 0.0])

        # === MPC INITIALIZATION WITH GP OPTIONS ===
        if CASADI_AVAILABLE:
            collect_data = os.getenv("GP_DATA_COLLECTION", "false").lower() == "true"
            use_gp_model = os.getenv("GP_USE_MODEL", "false").lower() == "true"
            model_path = os.getenv(
                "GP_MODEL_PATH",
                "/home/grandediw/ros2_px4_offboard_example_ws/gp_models/gp_mpc_trained_20251125_223658.pkl",
                #"/home/grandediw/ros2_px4_offboard_example_ws/gp_models/gp_model_20251119_030043.pkl",
            )

            if collect_data:
                # Data collection mode - nominal MPC + logging for offline GP
                self.get_logger().info("📊 Data collection mode enabled")
                self.mpc = QuadrotorMPC(dt=0.02, N=25, use_gp=False)
                if GP_AVAILABLE:
                    self.mpc.gp_dynamics = SimpleQuadrotorGP(max_data_points=2000)
                ...
                # GP-enhanced mode - load trained GP and use inside MPC
                self.get_logger().info("🧠 GP-enhanced mode enabled")
                self.mpc = QuadrotorMPC(dt=0.02, N=25, use_gp=True)
                ...
                if GP_AVAILABLE:
                    ...
                    if hasattr(self.mpc.gp_dynamics, "load_model"):
                        try:
                            if self.mpc.gp_dynamics.load_model(model_path):
                                model_loaded = True
                                self.get_logger().info(
                                    f"✅ GP model loaded via load_model(): {model_path}"
                                )
                        except Exception as e:
                            self.get_logger().warn(f"⚠️ load_model() failed: {e}")
                    ...
                    if not model_loaded:
                        try:
                            ...
                            self.mpc.gp_dynamics.gp_model = model_data["gp_model"]
                            self.mpc.gp_dynamics.is_trained = True
                            self.mpc.gp_dynamics.training_count = model_data.get(
                                "training_count", 1
                            )

                            model_loaded = True
                            self.get_logger().info(
                                f"✅ GP model loaded manually: {model_path}"
                            )
                        except Exception as e:
                            self.get_logger().error(
                                f"❌ Failed to load GP model manually: {e}"
                            )
                    ...
                    if model_loaded:
                        try:
                            if "SimpleGPEnhancedMPC" in globals():
                                self.mpc.gp_enhanced = SimpleGPEnhancedMPC(
                                    self.mpc.gp_dynamics, confidence_threshold=0.15
                                )
                            else:
                                self.mpc.gp_enhanced = self.mpc.gp_dynamics

                            stats = self.mpc.gp_dynamics.get_stats()
                            self.get_logger().info("📊 GP Model Info:")
                            self.get_logger().info(
                                f"   - Training samples: {stats.get('data_points', 'unknown')}"
                            )
                            self.get_logger().info(
                                f"   - Training iterations: {stats.get('training_iterations', 'unknown')}"
                            )
                            self.get_logger().info("   - Model ready for enhanced dynamics")
                        except Exception as e:
                            self.get_logger().error(
                                f"❌ Failed to initialize GP-enhanced wrapper: {e}"
                            )
                            model_loaded = False
                ...
                # Standard MPC mode: no GP, no data collection
                self.get_logger().info("📊 Standard MPC mode (no GP)")
                self.mpc = QuadrotorMPC(dt=0.02, N=25, use_gp=False)
                if GP_AVAILABLE:
                    # Still collect data for possible offline GP training
                    self.mpc.gp_dynamics = SimpleQuadrotorGP(max_data_points=800)
                    self.mpc.gp_enhanced = None
        else:
            self.get_logger().error("❌ CasADi not available - MPC disabled")
    ...
    def save_gp_dataset(self):
        """Save GP training data (if available) to CSV for offline training."""
        if self.mpc is not None and self.mpc.gp_dynamics is not None:
            try:
                self.mpc.gp_dynamics.save_dataset(self.gp_csv_path)
                self.get_logger().info(f"📁 Saved GP dataset to {self.gp_csv_path}")
            except Exception as e:
    ...
    def publish_rates_setpoint(self, rollrate=0.0, pitchrate=0.0, yawrate=0.0, thrust=0.5):
        ...
        normalized_thrust = float(np.clip(thrust, 0.0, 1.0))
        msg.thrust_body[0] = 0.0
        msg.thrust_body[1] = 0.0
    ...
    def calculate_reference_trajectory(self, t):
        """Calculate reference trajectory - Figure-8 pattern in x-y, fixed z offset"""
        A = float(self.pattern_amplitude)
        w = 2.0 * math.pi * float(self.pattern_frequency)

        ramp = math.tanh(max(t, 0.0) / 5.0)
    ...
    def get_current_state_vector(self):
        """Get current 12D state vector for logging and GP data."""
        return np.concatenate(
            [
                self.current_position,  # x,y,z
                self.current_velocity,  # vx,vy,vz
    ...
    def update_mpc_control(self, target_pos, target_yaw=0.0):
        ...
        if (
            ...
            and self.previous_control is not None
        ):
            self.mpc.add_training_data(self.previous_state, self.previous_control, current_state)

        # === MPC OPTIMIZATION ===
        accel_opt, X_opt = self.mpc.solve(current_state, target_pos, target_yaw)

        if accel_opt is not None:
            accel_des = accel_opt[:3]
            yawrate_des = accel_opt[3]

            accel_des = np.clip(accel_des, [-3.5, -3.5, -4.0], [3.5, 3.5, 6.0])
            yawrate_des = float(np.clip(yawrate_des, -0.8, 0.8))
        else:
            accel_des = np.array([0.0, 0.0, 0.0])
            yawrate_des = 0.0

        # === GEOMETRIC CONTROL ALLOCATION ===
        control_allocation = self._geometric_control_allocation(accel_des, target_yaw, yawrate_des)

        # Predicted next velocity (for logging)
        if X_opt is not None and X_opt.shape[1] > 1:
            predicted_vel = X_opt[3:6, 1]
        else:
            predicted_vel = self.current_velocity + accel_des * self.mpc.dt
    ...
    def _geometric_control_allocation(self, accel_des, target_yaw, yawrate_des):
        ...
        thrust_vector = accel_des + np.array([0.0, 0.0, g])
        thrust_magnitude = float(np.linalg.norm(thrust_vector))

        thrust_normalized = float(np.clip(thrust_magnitude / g, 0.25, 1.2))

        if thrust_magnitude > 0.1:
            thrust_unit = thrust_vector / thrust_magnitude
            pitch_cmd = -math.asin(float(np.clip(thrust_unit[0], -0.4, 0.4)))
            roll_cmd = math.asin(float(np.clip(thrust_unit[1], -0.4, 0.4)))
        else:
            roll_cmd = 0.0
            pitch_cmd = 0.0

        # === ATTITUDE PID CONTROLLER (outer loop) ===
        Kp_att = 3.2
        Ki_att = 0.6
        Kd_att = 0.6
        ...
        rollrate_cmd = float(np.clip(rollrate_cmd, -1.2, 1.2))
        pitchrate_cmd = float(np.clip(pitchrate_cmd, -1.2, 1.2))
        yawrate_cmd = float(np.clip(yawrate_cmd, -0.8, 0.8))
    ...
    def publish_monitoring_data(self, control_outputs, flight_time):
        ...
        control_msg = Float64MultiArray()
        control_msg.data = [
            float(self.position_setpoint[0]),
            float(self.position_setpoint[1]),
            float(self.position_setpoint[2]),
            float(self.current_position[0]),
            float(self.current_position[1]),
            float(self.current_position[2]),
            float(control_outputs["velocity_setpoint"][0]),
            float(control_outputs["velocity_setpoint"][1]),
            float(control_outputs["velocity_setpoint"][2]),
            float(self.current_velocity[0]),
            float(self.current_velocity[1]),
            float(self.current_velocity[2]),
            float(control_outputs["attitude_setpoint"][0]),
            float(control_outputs["attitude_setpoint"][1]),
            float(control_outputs["attitude_setpoint"][2]),
            float(self.current_attitude[0]),
            float(self.current_attitude[1]),
            float(self.current_attitude[2]),
            float(control_outputs["thrust"]),
            float(flight_time),
        ]
        self.control_outputs_pub.publish(control_msg)
        ...
        if current_time < 2.0:
            ...
            target_position = np.array(
                [
                    trajectory_offset[0],
                    trajectory_offset[1],
                    trajectory_offset[2] + self.takeoff_height,
                ]
            )
            ...
            if pos_error > 50.0:
                self.get_logger().warn(
                    f"🚨 LARGE POSITION ERROR: {pos_error:.1f}m - Switching to hover mode!"
                )
                target_position = np.array([0.0, 0.0, self.takeoff_height])
                self.position_setpoint = target_position

            # MPC expects relative offsets for the trajectory
            control_outputs = self.update_mpc_control(trajectory_offset, target_yaw)
            ...
            if int(flight_time) % 2 == 0 and flight_time > 0:
                ...
                vel_error = np.linalg.norm(
                    control_outputs["velocity_setpoint"] - self.current_velocity
                )
                att_error = np.linalg.norm(
                    control_outputs["attitude_setpoint"] - self.current_attitude
                )

                self.get_logger().info(f"🧠 MPC TRAJECTORY Control (t={flight_time:.1f}s):")
                self.get_logger().info(
                    "   📍 Pos: setpoint=(%.1f, %.1f, %.1f), current=(%.1f, %.1f, %.1f), error=%.3fm"
                    % (
                        self.position_setpoint[0],
                        self.position_setpoint[1],
                        self.position_setpoint[2],
                ...
                self.get_logger().info(
                    "   🏃 Vel: setpoint=(%.2f, %.2f, %.2f), current=(%.2f, %.2f, %.2f), error=%.3fm/s"
                    % (
                        control_outputs["velocity_setpoint"][0],
                        control_outputs["velocity_setpoint"][1],
                        control_outputs["velocity_setpoint"][2],
                ...
                self.get_logger().info(
                    "   🎪 Att: setpoint=(%.1f°, %.1f°, %.1f°), current=(%.1f°, %.1f°, %.1f°), error=%.1f°"
                    % (
                        math.degrees(control_outputs["attitude_setpoint"][0]),
                        math.degrees(control_outputs["attitude_setpoint"][1]),
                        math.degrees(control_outputs["attitude_setpoint"][2]),
                        math.degrees(self.current_attitude[0]),
                        math.degrees(self.current_attitude[1]),
                        math.degrees(self.current_attitude[2]),
                        math.degrees(att_error),
                    )
                )
                self.get_logger().info(
                    "   🎯 Rate: commands=(%.1f°/s, %.1f°/s, %.1f°/s)"
                    % (
                        math.degrees(control_outputs["rate_setpoint"][0]),
                        math.degrees(control_outputs["rate_setpoint"][1]),
                        math.degrees(control_outputs["rate_setpoint"][2]),
                    )
                )
                self.get_logger().info(f"   🚀 Thrust: {control_outputs['thrust']:.3f}")
                if "mpc_accelerations" in control_outputs:
                    acc = control_outputs["mpc_accelerations"]
                    self.get_logger().info(
                        "   🧠 MPC Accel: (%.2f, %.2f, %.2f) m/s²" % (acc[0], acc[1], acc[2])
                    )

                if self.mpc is not None and self.mpc.gp_dynamics is not None:
                    gp_stats = self.mpc.gp_dynamics.get_stats()
                    if gp_stats.get("is_trained", False):
                        self.get_logger().info(
                            "   🧠 GP: %s samples, trained (%s iters), used inside MPC dynamics"
                            % (
                                gp_stats.get("data_points", "unknown"),
                                gp_stats.get("training_iterations", "unknown"),
                            )
                        )
                    else:
                        self.get_logger().info(
                            "   📊 GP: %s samples (learning / offline)"
                            % gp_stats.get("data_points", "unknown")
                        )
...
def main(args=None):
    print("🎯 Starting Geometric Trajectory Controller...")
    print("📊 Control Architecture:")
    print("   GP-enhanced MPC -> geometric allocation -> PX4 rate/thrust control")
    print("✅ MPC with optional GP residuals in the prediction model")
    print("")
